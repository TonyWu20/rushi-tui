...
pub fn resolve_lang(lang: &str) -> Option<&'static str> {
    ...
    Some(match t.as_str() {
        ...
        "sql" | "swift" | "kotlin" | "elixir" | "zig" | "nim" | "vb" | "dockerfile" => return None,
        _ => return None,
    })
}
...
fn lang_defs() -> &'static [LangDef] {
    static DEFS: &[LangDef] = &[
        LangDef {
            ...
            keywords: &[
                "fn", "let", "mut", "const", "static", "pub", "use", "mod", "crate", "impl",
                "trait", "struct", "enum", "type", "where", "match", "if", "else", "loop", "while",
                "for", "in", "return", "break", "continue", "async", "await", "unsafe", "extern",
        ...
        LangDef {
            ...
            keywords: &[
                ...
                "case", "default", "goto", "sizeof", "typedef", "struct", "union", "enum", "const",
...
mod tests {
    ...
    fn unknown_language_is_plain() {
        ...
        assert_eq!(
            segs,
            vec![vec![(Style::default(), "hello world".to_string())]]
        );
    }
    ...
    fn string_literal_is_a_single_token() {
        ...
        let lit = segs[0].iter().find(|(_, t)| t.as_str() == r#""hi there""#);
    ...
    fn highlighted_tokens_carry_no_background() {
        ...
        for (st, t) in &segs[0] {
            assert!(st.bg.is_none(), "token {t:?} carries a background: {st:?}");
        }
