...
Status: open (2026-09-16). A follow-up request on the shipped
model-wait indicator. The base contract is
...
|----|----------|-------|--------|
| P1 | working-state | new test in `bin/tui/src/render.rs` (state derivation, bit, row text) | open (implementation not started) |
| P2 | wait-pending | the existing `working_row_shows_the_wait` test stays green, plus a no-stream-file row | open (implementation not started) |
| P3 | done-window | new test: `done` line read, no settle, state stays `working` | open (implementation not started) |
| P4 | settle-fallback | new test: settle clears the buffer, the marker-derived state returns | open (implementation not started) |
| P5 | unknown-marker | new test: no marker + open stream buffer stays `running-unknown` | open (implementation not started) |
| P6 | stale-file | new test: running bit clear + stream file present is `idle` | open (implementation not started) |
| P7 | timer-continuity | new test: the span during `working` counts from the `wait` marker | open (implementation not started) |
