...
mod tests {
    ...
    fn non_image_returns_none() {
        ...
        assert!(image_body_rows(&value, 40).is_none());
    }

    #[test]
    fn image_data_renders_lines() {
        ...
        let rows = image_body_rows(&value, 40);
        assert!(rows.is_some(), "expected image rows");
        let rows = rows.unwrap();
        // Header + N image lines.
        assert!(
            rows.len() >= 2,
            "header + at least one image row, got {}",
            rows.len()
        );
        // The header mentions the dimensions.
        let header_text: String = rows[0].iter().map(|(_, s)| s.as_str()).collect();
        assert!(
            header_text.contains("4x4"),
            "header should mention 4x4: {header_text}"
        );
    }
    ...
    fn luminance_values() {
        ...
        let lum = luminance(&green);
        assert!(lum > 140 && lum < 160, "green lum: {lum}");
    }

    #[test]
    fn halfblock_char_mapping() {
        // Both dark: space.
        let black = image::Rgba([0, 0, 0, 255]);
        let (ch, _, _) = halfblock_char(&black, &black);
        assert_eq!(ch, ' ');

        // Both bright: full block.
        let white = image::Rgba([255, 255, 255, 255]);
        let (ch, _, _) = halfblock_char(&white, &white);
        assert_eq!(ch, '\u{2588}');

        // Top brighter: upper half.
        let red = image::Rgba([200, 0, 0, 255]);
        let dark = image::Rgba([10, 10, 10, 255]);
        let (ch, fg, _) = halfblock_char(&red, &dark);
        assert_eq!(ch, '\u{2580}');
        assert_eq!(fg, Color::Rgb(200, 0, 0));

        // Bottom brighter: lower half.
        let (ch, fg, _) = halfblock_char(&dark, &red);
        assert_eq!(ch, '\u{2584}');
        assert_eq!(fg, Color::Rgb(200, 0, 0));
    }
}
